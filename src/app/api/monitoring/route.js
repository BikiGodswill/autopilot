...
import { getMonitoringSettings, upsertMonitoringSettings } from "@/services/monitoring/monitoringService";
...
export async function GET(request) {
  ...
  const { data, error } = await getMonitoringSettings(supabase, websiteId);
...
export async function POST(request) {
  ...
  const { data, error } = await upsertMonitoringSettings(supabase, body.websiteId, body);
  if (error) return NextResponse.json(apiError("DB_ERROR", error.message), { status: 500 });
